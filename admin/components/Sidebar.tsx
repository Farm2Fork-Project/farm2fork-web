'use client'

import React, { useState } from 'react'
import Link from 'next/link'
import { usePathname, useRouter } from 'next/navigation'
import {
  LayoutDashboard,
  Store,
  Wheat,
  BookOpen,
  TrendingUp,
  Settings,
  SlidersHorizontal,
  HelpCircle,
  Leaf,
  LogOut,
  MessageSquare,
} from 'lucide-react'
import GenerateReportModal from './GenerateReportModal'

const navItems = [
  { id: 'dashboard', href: '/dashboard', title: 'Dashboard', icon: LayoutDashboard },
  { id: 'users', href: '/users', title: 'User Management', icon: Settings },
  { id: 'products', href: '/products', title: 'Products', icon: Store },
  { id: 'orders', href: '/orders', title: 'Orders', icon: BookOpen },
  { id: 'loans', href: '/loans', title: 'Loan Applications', icon: Wheat },
  { id: 'moderation', href: '/moderation', title: 'Moderation', icon: MessageSquare },
  { id: 'audit', href: '/audit-logs', title: 'Audit Logs', icon: TrendingUp },
  { id: 'config', href: '/config', title: 'System Config', icon: SlidersHorizontal },
]

export default function Sidebar() {
  const pathname = usePathname() || '/'
  const router = useRouter()
  const [showReport, setShowReport] = useState(false)
  const [showLogout, setShowLogout] = useState(false)

  const handleLogout = () => {
    localStorage.removeItem('role')
    localStorage.removeItem('userName')
    localStorage.removeItem('avatarUrl')
    router.push('/login')
  }

  return (
    <>
      <aside className="sidebar">
        {/* Logo */}
        <div className="sidebar-logo">
          <h1>
            <Leaf size={22} />
            Farm2Fork
          </h1>
          <p>Supply Chain Admin</p>
        </div>

        {/* Navigation */}
        <nav className="sidebar-nav">
          {navItems.map((item) => {
            const isActive =
              item.href === '/'
                ? pathname === '/'
                : pathname.startsWith(item.href)
            const Icon = item.icon

            return (
              <Link
                key={item.id}
                href={item.href}
                className={`sidebar-link${isActive ? ' active' : ''}`}
              >
                <span className="icon"><Icon size={18} /></span>
                {item.title}
              </Link>
            )
          })}
        </nav>

        {/* Bottom */}
        <div className="sidebar-bottom">
          <button
            className={`sidebar-report-btn${showReport ? ' active' : ''}`}
            onClick={() => setShowReport(true)}
          >
            Generate Report
          </button>
          <button
            onClick={() => setShowLogout(true)}
            className="sidebar-bottom-link"
            style={{ width: '100%', background: 'none', border: 'none', textAlign: 'left', cursor: 'pointer', fontFamily: 'inherit' }}
          >
            <span className="icon"><LogOut size={16} /></span>
            Logout
          </button>
        </div>
      </aside>

      {showReport && (
        <GenerateReportModal onClose={() => setShowReport(false)} />
      )}

      {showLogout && (
        <div className="modal-overlay" onClick={() => setShowLogout(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <div className="modal-header">
              <h2>Confirm Logout</h2>
              <button className="modal-close" onClick={() => setShowLogout(false)}>
                ✕
              </button>
            </div>
            <p style={{ color: 'var(--text-muted)' }}>Are you sure you want to log out?</p>
            <div className="modal-actions">
              <button className="btn btn-secondary" onClick={() => setShowLogout(false)}>
                Cancel
              </button>
              <button className="btn btn-primary" style={{ backgroundColor: 'var(--error-red)' }} onClick={handleLogout}>
                Logout
              </button>
            </div>
          </div>
        </div>
      )}
    </>
  )
}
